...
function handle_captcha($type, $func, &$param = null)
{
    ...
    switch ($CAPTCHA->check_CAPTCHA($type)) {
        case 0:
            ...
            break;
        case 1:
            $TEMPLATE->add_message(lang('captcha_wronganswer'));
            break;
        case 2:
            $TEMPLATE->add_message(lang('captcha_wrongid'));
            break;
        default:
            break;
    }
...
function add_quote_do_inner()
{
    ...
    $res =& $db->query("INSERT INTO " . db_tablename('quotes') . " (quote, rating, flag, queue, date, note, ip) VALUES(" . $db->quote($quotxt) . ", 0, " . $flag . ", " . $CONFIG['moderated_quotes'] . ", '" . mktime() . "', " . $db->quote($note) . ", '" . $_SERVER['REMOTE_ADDR'] . "')");
    if (DB::isError($res)) {
        //TODO: Add Q0 Shrug
